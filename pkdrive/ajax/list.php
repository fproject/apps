...
try {
	...
	if (is_array($mimetypeFilters) && count($mimetypeFilters)) {
		...
		foreach ($mimetypeFilters as $mimetypeFilter) {
			$files = array_merge($files, \OCA\PkDrive\Helper::getFiles($dir, $sortAttribute, $sortDirection, $mimetypeFilter));
		}

		// sort the files accordingly
		$files = \OCA\PkDrive\Helper::sortFiles($files, $sortAttribute, $sortDirection);
	} else {
...
//		$files = \OCA\Files\Helper::getFiles($dir, $sortAttribute, $sortDirection);
	...
	$files = \OCA\PkDrive\Helper::populateTags($files);
	$data['directory'] = $dir;
	$data['files'] = \OCA\PkDrive\Helper::formatFileInfos($files);
